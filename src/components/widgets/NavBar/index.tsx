...
const NavBar = () => {
	...
	const getNavItem = () => {
		switch (pathname) {
			...
			case '/chat/new':
				return <div
					className={'text-medium sm:text-lg flex items-center gap-1 bg-[#2A2A2A] hover:bg-[#3b3b39] transition-colors cursor-pointer h-[40px] rounded-3xl px-3 py-1.5'}>
			...
			case '/auth/new-password':
				return <Link href={'/auth'}
										 className={'font-bold text-[#27ACC9] hover:text-[#32cbed] transition-colors text-sm sm:text-lg'}>Log
					in</Link>
			case '/auth/onboard':
				return <Link href={'/auth'}
										 className={'font-bold text-[#27ACC9] hover:text-[#32cbed] transition-colors hidden sm:flex text-sm sm:text-lg'}>Log
					in</Link>
			case '/auth':
				return <Link href={'/auth/register'}
										 className={'font-bold text-[#27ACC9] hover:text-[#32cbed] transition-colors text-sm sm:text-lg'}>Sign
					up</Link>
			default:
				return null
		}
